...
class DSPProcessor:
    ...
    def process_project(self, project_identifier: str, output_dir: str = None):
        ...
        project_name = os.path.basename(project_identifier).replace(".md", "") if os.path.isfile(project_identifier) else project_identifier
        ...
        with Progress(
            ...
            task = progress.add_task(description="Generating DSP...", total=len(template))
            ...
            for section in template:
                ...
                # AI: Generate
                _, content = self.generator.generate_section(title, body, summary, examples)
